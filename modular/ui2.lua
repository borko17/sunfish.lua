...
-- Router: dispatches to the shaded-block "fancy" renderer when enabled,
-- otherwise falls through to the original bordered/lettered renderer below.
-- Every call site in main.lua/challenge.lua stays untouched.
function printboard(board, lastMove, checkers, guards, isMate, hints)
   if USE_FANCY_BOARD then
      return printboard_fancy(board, lastMove, checkers, guards, isMate, hints)
   end
   return printboard_classic(board, lastMove, checkers, guards, isMate, hints)
end

function printboard_classic(board, lastMove, checkers, guards, isMate, hints)
...
-- Fancy shaded-block renderer ("c" toggle / USE_FANCY_BOARD).
-- No ANSI color -- Yantra's terminal renders raw escape bytes as literal
-- text (confirmed on-device), so squares are told apart with shade glyphs
-- instead of a colored background: ▓ dark square, ░ light square.
-- Pieces always use the filled unicode glyphs (♚♛♜♝♞♟) regardless of side,
-- with an uppercase/lowercase letter fallback for non-unicode mode, since
-- there's no color to tell white from black by outline vs filled glyph here.
-- Board markers (check/guard/highlight/hint) reuse the same bracket
-- convention as printboard_classic so the two modes stay legible together.
function printboard_fancy(board, lastMove, checkers, guards, isMate, hints)
   checkers = checkers or {}
   guards = guards or {}
   hints = hints or {}
   local highlight = {}
   ...
   local darkShade  = '\xe2\x96\x93'  -- ▓
   local lightShade = '\xe2\x96\x91'  -- ░
   ...
   local l = strsplit(board, '\n')
   print("")
   local topBorder    = "  \xe2\x95\x94" .. string.rep('\xe2\x95\x90', 26) .. "\xe2\x95\x97"  -- ╔ ... ╗
   local sideBorder    = '\xe2\x95\x91'                                                        -- ║
   local bottomBorder = "  \xe2\x95\x9a" .. string.rep('\xe2\x95\x90', 26) .. "\xe2\x95\x9d"  -- ╚ ... ╝
   ...
   for k = 3, 10 do
      ...
      for i = 2, 9 do
         ...
         local file = i - 1
         local idx = (k - 1) * 10 + (i - 1)
         local isDark = (file + rank) % 2 == 1
         local shade = isDark and darkShade or lightShade

         local sym
         if c == '.' then
            sym = shade
         elseif USE_UNICODE_PIECES then
            -- Both sides use the SAME filled glyph (no color to distinguish
            -- them by outline vs filled here); uppercase/lowercase in the
            -- underlying board string is still what drives move logic.
            sym = fancyBlack[c] or fancyWhite[c] or c
         else
            sym = c
         end

         if hints[idx] then
            if #line > 0 then line[#line] = line[#line]:gsub(" $", "") end
            local q = hints[idx].quote
            table.insert(line, q .. sym .. q .. " ")
         elseif checkers[idx] then
            if #line > 0 then line[#line] = line[#line]:gsub(" $", "") end
         ...
         elseif c == '.' then
            -- empty square: repeat the shade glyph to fill the cell width
            -- (keeps columns aligned the same way piece cells do)
            table.insert(line, shade .. shade .. " ")
         else
            table.insert(line, sym .. "  ")
         end
      end
