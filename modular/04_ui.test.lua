...
-- (even), index 27 = step 26 (completely winning). Two extra steps sit
-- outside that range: [-1] for a barely-there edge under 100, and [27] for
-- a near-mate score at/above 2700. Same text is used regardless of which
-- side is ahead - only the prefix (below) says who.
...
local SCORE_STEP_MIN = -1
local SCORE_STEP_MAX = 27

-- Converts a raw engine score into a step on the evaluation scale.
-- 0 -> even. 1-99 -> -1 ("barely noticeable", a sub-step below the regular
-- 100-point ticks so it doesn't get inflated to a full step). 100-2699 ->
-- one step per 100 points (100-199 = step 1, ..., 2600-2699 = step 26).
-- 2700+ -> step 27 ("almost checkmate"). Kept as the single source of
-- truth - the border tick count below just calls this instead of
-- recomputing it.
local function scoreStep(score)
   local mag = math.abs(score)
   if mag <= 0 then return 0 end
   if mag < 100 then return SCORE_STEP_MIN end
   local step = math.floor(mag / 100)
...
-- it. Each 100 points of |score| earns one tick mark: 100-199 -> 1 tick,
-- 200-299 -> 2 ticks, ..., 2600-2699 -> 26 ticks, counted from the LEFT.
-- Two sub-step markers sit outside that run: 1-99 uses a faint marker
-- (SCORE_TICK_SYMBOL_MIN) in the first cell instead of a full tick, and
-- 2700+ uses an emphatic marker (SCORE_TICK_SYMBOL_MAX) in the last cell.
-- Only the border's color (green for you, red for Sunfish) shows who has
-- the advantage. Corners (+ or the unicode corner glyphs) are never
-- overwritten by the indicator. The border keeps the same indent as the
-- rest of the board (bottom border, ranks) so the left + always lines up
-- with the | below it, colored or not.
local SCORE_TICK_SYMBOL = "#"
local SCORE_TICK_SYMBOL_MIN = "." -- sub-100 edge: faint marker, first cell
local SCORE_TICK_SYMBOL_MAX = "!" -- 2700+: emphatic marker, last cell
...
-- Returns which cell (1..INNER_WIDTH) gets the marker, and which symbol.
-- Steps 1-26 map 1:1 to cells 1-26 with the regular tick symbol. Step -1
-- (sub-100 edge) and step 27 (2700+) don't have their own cell - they
-- reuse the first/last cell with a distinct symbol instead.
local function scoreTickCell(score)
   local step = scoreStep(score)
   if step == SCORE_STEP_MIN then
      return 1, SCORE_TICK_SYMBOL_MIN
   elseif step == SCORE_STEP_MAX then
      return INNER_WIDTH, SCORE_TICK_SYMBOL_MAX
   end
   return step, SCORE_TICK_SYMBOL
end
...
local function buildScoreBorderLine(unicodeMode, corner)
   ...
   if hasIndicator then
      local cell, symbol = scoreTickCell(score)
