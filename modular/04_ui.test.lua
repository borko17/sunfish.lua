...
-- Converts a centipawn score into a 0-26 step on the evaluation scale.
-- Every 100 centipawns is one step; magnitude is capped at 26.
local function scoreStep(score)
   local step = math.floor(math.abs(score) / 100)
...
function printEngineScore()
   ...
   if score < 0 then
      echoS(string.format("\xe2\x80\x8b     %s (You)", text))
   elseif score > 0 then
      echoE(string.format("\xe2\x80\x8b     %s (Sunfish)", text))
   else
