...
-- DISPLAY_MODE_STEP (0-7) je jedini izvor istine; sve ostalo se izvodi iz njega.
...
local INNER_WIDTH = 26
local BORDER_LEAD_SPACES = "  "
local BORDER_LEAD_SPACES_COLORED = "\xe2\x80\x8b  "
...
-- POPRAVLJENA FUNKCIJA: sada uvek vraća boju ako score postoji
local function buildScoreBorderLine(unicodeMode, corner)
   ...
   if hasIndicator then
      ...
      -- POPRAVKA: eksplicitno proveri da li je score negativan
      if score < 0 then
         colorFn = echoS  -- zeleno za igrača
      else
         colorFn = echoE  -- crveno za Sunfish
      end
   end
   ...
   for k = kFrom, kTo, kStep do
      ...
      for i = iFrom, iTo, iStep do
         ...
         elseif usingUnicodePieces() then
            local isWhitePiece = c:match('%u') ~= nil
...
function capturedAt(pos, move)
   ...
   if qb >= 97 and qb <= 122 then
      return string.char(qb - 32)
   end
   if pb == 80 and (j - i == N+W or j - i == N+E) and qb == 46 and j == pos.ep then
      return 'P'
   end
   return nil
end
...
function isPawnMove(pos, move)
   local i = move[0 + __1]
   return pos.board[i + __1] == 80
end

function isPromotionMove(pos, move)
   local i = move[0 + __1]
   local j = move[1 + __1]
   return pos.board[i + __1] == 80 and A8 <= j and j <= H8
end
...
function findKingGuards(p, checkers)
      ...
      elseif ptype == 'N' or ptype == 'K' then
         ...
         for _, d in ipairs(offs) do
            ...
            while true do
               ...
               if cb == nil or cb == 32 or cb == 10 then break end
               if j == target then return true end
               if cb ~= 46 then break end
               j = j + d
            end
         end
         return false
   ...
   for _, d in ipairs(directions.K) do
      ...
      if cb and cb ~= 32 and cb ~= 10 and not (cb >= 65 and cb <= 90) then
         for i = 1 - __1, #board - __1 do
            local pcb = board[i + __1]
            if pcb and pcb >= 97 and pcb <= 122 and not checkers[i] then
...
function saveGame(pos, lastMove, capturedByUser, capturedByEngine, whiteMoves, blackMoves, halfmoveClock, nextToMove, moveHistory, startingBoard, extra)
   ...
   local code = 'c:' .. wcStr .. '|bc:' .. bcStr .. '|ep:' .. epStr ..
                ...
                '|side:' .. (PLAYER_IS_BLACK and 'b' or 'w') ..
