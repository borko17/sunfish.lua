...
-- search() always returns a score from the perspective of whoever is "to
-- move" in the position it was given (Sunfish convention: positive = good
-- for the side to move). The eval bar wants an absolute score (positive =
-- good for White), so convert using whether White was the side to move in
-- that particular search() call.
function toWhiteScore(score, sideToMoveIsWhite)
   if sideToMoveIsWhite then
      return score
   else
      return -score
   end
...
-- Same mate-score constants search() uses (not exposed globally there).
EVAL_BAR_MATE_LOWER = 60000 - (13 * 2529)
EVAL_BAR_MATE_UPPER = 60000 + (10 * 2529)

-- Builds the 8 row-segments (top-to-bottom) of a vertical eval bar, similar to
-- the sidebar seen on sites like Lichess/Chess.com: filled from the middle
-- toward whichever side is ahead. `score` is centipawns from White's
-- perspective (positive = White better). Mate scores clamp to a full bar.
function evalBarSegments(score)
   local FULL = 8
   local CLAMP_CP = 1000 -- centipawns treated as "off the scale" (bar maxed out)
   local s = score
   if s >= EVAL_BAR_MATE_LOWER then s = CLAMP_CP end
   if s <= -EVAL_BAR_MATE_LOWER then s = -CLAMP_CP end
   if s > CLAMP_CP then s = CLAMP_CP end
   if s < -CLAMP_CP then s = -CLAMP_CP end

-- Map [-CLAMP_CP, CLAMP_CP] to how many of the 8 segments (top-down) are
-- "White" (filled) vs "Black" (empty), rounding so 0.0 shows an even split.
   local whiteFrac = (s + CLAMP_CP) / (2 * CLAMP_CP) -- 0..1, 1 = all White
   local whiteSegsFromBottom = math.floor(whiteFrac * FULL + 0.5)
   if whiteSegsFromBottom < 0 then whiteSegsFromBottom = 0 end
   if whiteSegsFromBottom > FULL then whiteSegsFromBottom = FULL end

   local blackChar, whiteChar
   if usingUnicodePieces() then
      blackChar, whiteChar = '\xe2\x96\x91', '\xe2\x96\x88' -- ░ (dark side) / █ (light side)
   else
      blackChar, whiteChar = '.', '#'
   end
   ...
   local segs = {}
   for row = 1, FULL do
-- row=1 is the top of the board (rank 8 side); White's filled segments grow
-- from the bottom (rank 1 side) upward, matching a real eval bar.
      local fromTop = FULL - row + 1
      if fromTop <= whiteSegsFromBottom then
         segs[row] = whiteChar
      else
         segs[row] = blackChar
      end
...
-- Formats the eval score for the small numeric readout under/beside the bar.
function formatEvalScore(score)
   if score >= EVAL_BAR_MATE_LOWER then
      local pliesToMate = EVAL_BAR_MATE_UPPER - score
      return "#" .. tostring(math.ceil(pliesToMate / 2))
   elseif score <= -EVAL_BAR_MATE_LOWER then
      local pliesToMate = EVAL_BAR_MATE_UPPER + score
      return "#-" .. tostring(math.ceil(pliesToMate / 2))
   else
...
-- Set by main() right after each search() call (converted to White's-
-- perspective centipawns). printboard()/displayPosition() fall back to this
-- when no explicit evalScore is passed, so the bar reflects the last known
-- evaluation without threading a new parameter through every call site.
LAST_EVAL_SCORE = LAST_EVAL_SCORE or nil

function printboard(board, lastMove, checkers, guards, isMate, hints, evalScore)
   ...
   if evalScore == nil then evalScore = LAST_EVAL_SCORE end
   local evalSegs = (SHOW_EVAL_BAR and evalScore) and evalBarSegments(evalScore) or nil
...
elseif highlight[idx] then
   ...
   table.insert(line, sym .. "  ")
      ...
      if evalSegs then
         local rowInDisplay = k - kFrom + 1 -- 1..8, top-to-bottom as printed
         table.insert(line, "  " .. evalSegs[rowInDisplay] .. evalSegs[rowInDisplay])
      end
      print(table.concat(line))
   end
   print(bottomBorder)
   if PLAYER_IS_BLACK then
      print("     h  g  f  e  d  c  b  a" .. (evalSegs and ("   " .. formatEvalScore(evalScore)) or ""))
   else
      print("     a  b  c  d  e  f  g  h" .. (evalSegs and ("   " .. formatEvalScore(evalScore)) or ""))
   end
   print("")
end
...
-- evalScore (optional): centipawns from White's perspective, drawn as a
-- vertical eval bar to the right of the board.
function displayPosition(pos, lastMove, capturedByUser, capturedByEngine, blackMoves, evalScore)
   if evalScore == nil then evalScore = LAST_EVAL_SCORE end
   if lastMove then
   ...
   printboard(arrayToBoard(pos.board), lastMove, checkers, guards, isMate, nil, evalScore)
