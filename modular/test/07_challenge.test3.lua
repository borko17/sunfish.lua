...
-- Capture full pre-move state for 'z' (undo), right before this move is applied. One level only: overwrites any previous undoSnapshot.
      ...
      local peekScore = pos.score -- incremental material/PST score from move_impl(); matches setEngineScore()'s sign convention directly (verified: negative after a good capture for the player, e.g. -198 after winning a free pawn) - captured just before pos.score is zeroed for search()'s own baseline below
      pos.score = 0
      gameHistory[tpKey(pos)] = true
      positionCounts[tpKey(pos)] = (positionCounts[tpKey(pos)] or 0) + 1
