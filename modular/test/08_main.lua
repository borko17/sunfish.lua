...
-- Show the initial board first, before announcing that Sunfish is thinking.
      ...
      LAST_EVAL_SCORE = toWhiteScore(score, true) -- Sunfish is playing White here
...
while true do
   ...
   elseif crdn == 'e' then
      ...
      LAST_EVAL_SCORE = toWhiteScore(analyzeScore, not PLAYER_IS_BLACK) -- pos is from the human player's perspective here
...
assert(score)
LAST_EVAL_SCORE = toWhiteScore(score, PLAYER_IS_BLACK) -- Sunfish plays the side opposite the human
            if PROFILE_PRINT_ENABLED then
               printProfile(elapsed, reachedDepth, usedNodes)
            end
            if enginemove and not isLegalMove(rotated, enginemove) then
