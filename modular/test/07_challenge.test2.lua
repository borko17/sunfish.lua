...
-- Capture full pre-move state for 'z' (undo), right before this move is applied. One level only: overwrites any previous undoSnapshot.
      ...
      local peekScore = -pos.score -- incremental material/PST score from move_impl() has the opposite sign convention from search()'s return value for the same position (verified empirically), so it's negated here to match what setEngineScore()/the border expect; captured just before pos.score is zeroed for search()'s own baseline below
      pos.score = 0
      gameHistory[tpKey(pos)] = true
      positionCounts[tpKey(pos)] = (positionCounts[tpKey(pos)] or 0) + 1
...
-- Snapshot for 's<N>'; pos is in Black's rotated view here, so store the White-view rotation to match saveGame()/loadGame().
      ...
      -- Score display refreshed here using peekScore - the incremental
      -- material/positional score already tracked on the Position object
      -- (updated on every move_impl(), no search involved), captured just
      -- before pos.score was zeroed for search()'s own baseline above. This
      -- is instant (no MTD-bi node-budget issue like the old SCORE_PEEK_NODES
      -- search had) but it's a static eval, not a searched score: no
      -- lookahead, so it won't see tactics beyond material/PST terms.
      -- Sunfish's real search() below still overwrites it with the searched
      -- score once it moves.
